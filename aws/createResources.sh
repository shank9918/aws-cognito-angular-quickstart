...
BUCKET_NAME=$(echo "$ROOT_NAME" | tr '[:upper:]' '[:lower:]').platform
TABLE_NAME=bmc_logintrials
...
createCognitoResources() {
    ...
    $aws_cmd cognito-identity create-identity-pool --identity-pool-name $IDENTITY_POOL_NAME --allow-unauthenticated-identities --region $REGION| grep IdentityPoolId | awk '{print $2}' | xargs |sed -e 's/^"//'  -e 's/"$//' -e 's/,$//' > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/poolId
    IDENTITY_POOL_ID=$(cat /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/poolId)
    echo "Created an identity pool with id of " $IDENTITY_POOL_ID

    # Create an IAM role for unauthenticated users
    cat unauthrole-trust-policy.json | sed 's/IDENTITY_POOL/'$IDENTITY_POOL_ID'/' > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/unauthrole-trust-policy.json
    $aws_cmd iam create-role --role-name $ROLE_NAME_PREFIX-unauthenticated-role --assume-role-policy-document file://./tmp/unauthrole-trust-policy.json > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/iamUnauthRole
    if [ $? -eq 0 ]
    then
        echo "IAM unauthenticated role successfully created"
    else
        echo "Using the existing role ..."
        $aws_cmd iam get-role --role-name $ROLE_NAME_PREFIX-unauthenticated-role  > file://./tmp/iamUnauthRole
    ...
    $aws_cmd iam put-role-policy --role-name $ROLE_NAME_PREFIX-unauthenticated-role --policy-name CognitoPolicy --policy-document file://./tmp/unauthrole.json

    # Create an IAM role for authenticated users
    cat authrole-trust-policy.json | sed 's/IDENTITY_POOL/'$IDENTITY_POOL_ID'/' > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/authrole-trust-policy.json
    $aws_cmd iam create-role --role-name $ROLE_NAME_PREFIX-authenticated-role --assume-role-policy-document file://./tmp/authrole-trust-policy.json > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/iamAuthRole
    if [ $? -eq 0 ]
    then
        echo "IAM authenticated role successfully created"
    else
        echo "Using the existing role ..."
        $aws_cmd iam get-role --role-name $ROLE_NAME_PREFIX-authenticated-role  > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/iamAuthRole
    ...
    cat authrole.json | sed 's~DDB_TABLE_ARN~'$DDB_TABLE_ARN'~' > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/authrole.json
    ...
    $aws_cmd cognito-idp create-user-pool --pool-name $POOL_NAME --auto-verified-attributes email --policies file://./tmp/user-pool-policy.json --region $REGION > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/$POOL_NAME-create-user-pool
    USER_POOL_ID=$(grep -E '"Id":' /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/$POOL_NAME-create-user-pool | awk -F'"' '{print $4}')
    echo "Created user pool with an id of " $USER_POOL_ID

    # Create the user pool client
    $aws_cmd cognito-idp create-user-pool-client --user-pool-id $USER_POOL_ID --no-generate-secret --client-name webapp --region $REGION > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/$POOL_NAME-create-user-pool-client
    USER_POOL_CLIENT_ID=$(grep -E '"ClientId":' /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/$POOL_NAME-create-user-pool-client | awk -F'"' '{print $4}')
    ...
    $aws_cmd cognito-identity update-identity-pool --allow-unauthenticated-identities --identity-pool-id $IDENTITY_POOL_ID --identity-pool-name $IDENTITY_POOL_NAME \
        ...
        > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/$IDENTITY_POOL_ID-add-user-pool

    # Update cognito identity with the roles
    UNAUTH_ROLE_ARN=$(perl -nle 'print $& if m{"Arn":\s*"\K([^"]*)}' /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/iamUnauthRole | awk -F'"' '{print $1}')
    AUTH_ROLE_ARN=$(perl -nle 'print $& if m{"Arn":\s*"\K([^"]*)}' /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/iamAuthRole | awk -F'"' '{print $1}')
...
createDDBTable() {
    ...
    $aws_cmd dynamodb create-table \
        ...
        --region $REGION \
        > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/dynamoTable
    ...
    then
        ...
        $aws_cmd dynamodb describe-table --table-name $TABLE_NAME > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/dynamoTable
    fi

    DDB_TABLE_ARN=$(perl -nle 'print $& if m{"TableArn":\s*"\K([^"]*)}' /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/dynamoTable | awk -F'"' '{print $1}')
}
...
createS3Bucket() {
    # Create the bucket
    $aws_cmd s3 mb s3://$BUCKET_NAME/ --region $REGION 2>/c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/s3-mb-status
    status=$?
    ...
    then
        ...
        if grep "BucketAlreadyOwnedByYou" /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/s3-mb-status > /dev/null
        then
...
uploadS3Bucket() {
    ...
    cat s3-bucket-policy.json | sed 's/BUCKET_NAME/'$BUCKET_NAME'/' > /c/Shashank/aws/aws-cognito-angular-quickstart/aws/tmp/s3-bucket-policy.json
